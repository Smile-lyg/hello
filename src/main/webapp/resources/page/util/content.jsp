            ...
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <input type="text" name="id" lay-verify="required" autocomplete="off"
                           class="layui-input" readonly id="userId">
                </div>
            </div>
            <div class="layui-inline">
                <label class="layui-form-label">用户名:</label>
...
<%--                        <option value="3" selected>汽车管理员</option>--%>
                    </select>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
        ...
        function openAddUser() {
            mainIndex = layer.open({
                ...
                , success: function (index){
                    ...
                    url = '/user/addUser';
                    // 获取并填入id
                    var res = $.post('/user/getId', function (obj) {
        ...
        // 下拉菜单里添加元素
        function selectRole() {
            $.post('/role/roleList', {'page': 1, 'limit': 0}, function (data) {
                roleData = data.data;
                $.each(roleData, function (index, item) {
                    $('#selectRole').append(new Option(item.explain, item.role));
                });
